...
def check_ffmpeg():
    try:
        ...
        log("FFmpeg не найден в PATH. Установите ffmpeg и перезапустите.")
        sys.exit(1)
...
def build_ffmpeg_cmd(img_path: Path, out_path: Path, duration: int, width: int, height: int, fps: int) -> list:
    # Размытый фон + оригинал по центру под вертикаль 1080x1920
    w, h = width, height
    vf = (
...
def wait_until_stable(path: Path, checks: int = 5, delay: float = 0.2):
    """Ждём, пока файл допишется: размер стабилен N раз подряд."""
    last_size = -1
    stable = 0
    for _ in range(100):  # максимум ~20 сек
        try:
...
def process_file_sync(path: Path, settings: Settings):
    try:
        ...
        video_path = convert_image_to_video(
            path, settings.ready_dir,
            settings.duration, settings.width, settings.height, settings.fps
        )
...
def start_watcher(settings: Settings) -> Observer:
    event_handler = InboxHandler(settings)
    observer = Observer()
...
def main():
    ...
    try:
        if SETTINGS.bot_token:
            # Блокирующий запуск бота, watcher будет работать параллельно
            ...
            log("Telegram‑бот запущен (polling).")
            app.run_polling(close_loop=False, allowed_updates=Update.ALL_TYPES)
        else:
            log("BOT_TOKEN не задан — бот не поднимался. Watcher работает. Нажмите Ctrl+C для выхода.")
            while True:
                time.sleep(1)
